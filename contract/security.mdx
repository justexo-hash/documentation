...
- **The operator key may be compromised** — admin can rotate via `setOperator()`. In-flight rentals are unaffected; the new operator can release them.
- **The admin key may be compromised** — game over for operator rotation. We recommend admin = Safe multisig in production to make compromise infeasible.
