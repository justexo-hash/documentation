...
Payments are escrowed by a Solidity contract on [Base](https://base.org) and paid in [native USDC](https://www.circle.com/blog/native-usdc-on-base). That gets you three things you don't get with a Stripe-only marketplace:
...
3. **Trust-minimized fallback.** If our backend disappears, anyone can call `release` on the contract after a rental's `endTime` to settle stuck funds.
